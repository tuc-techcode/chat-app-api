...
class Message_Controller extends Base_Controller
{
  ...
  public function sendRequestApproval(int $senderId)
  {
    try {
      $this->messageRepository->startTransaction();
      ...
      if (!$isParticipant) {
        throw new Exception("User is not part of this conversation", 400);
      }
      ...
      $requestApproval = $this->requestApprovalRepository->createRequestApproval(
        $message['id'],
        $senderId,
        $approverId,
        $priorityLevel,
      ...
      if (!$requestApproval) {
        throw new Exception("An error has occured while creating request approval", 400);
      }
